...
def windows_natural_sort_key(s):
    ...
    for part in parts:
        if part:  # 忽略空字符串
            sort_key.append(convert_part(part))
    
    return sort_key


def enhanced_natural_sort_key(filename):
    """
    增强的自然排序键函数，更好地处理各种文件名格式
    支持：
    - 数字排序（1, 2, 10, 11 而不是 1, 10, 11, 2）
    - 中文字符
    - 特殊字符
    - 大小写不敏感
    """
    ...
    # 使用Windows风格的自然排序
    return windows_natural_sort_key(name_without_ext)
...
def merge_images_to_pdf_from_folder(folder_path, output_pdf_path, uniform_size=True, resize_method="fit"):
    ...
    try:
        ...
        # 使用增强的自然排序，仿照Windows资源管理器
        filenames = sorted(filenames, key=enhanced_natural_sort_key)
...
def merge_images_to_pdf_from_zip(zip_path, output_pdf_path, uniform_size=True, resize_method="fit"):
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            ...
            # 使用增强的自然排序，仿照Windows资源管理器
            # 对于zip文件中的路径，我们按照完整路径排序，但主要关注文件名部分
            image_files = sorted(image_files, key=lambda x: enhanced_natural_sort_key(os.path.basename(x)))
...
def find_items_to_process(base_dir):
    ...
    try:
        ...
        # 使用增强的自然排序
        all_items = sorted(all_items, key=enhanced_natural_sort_key)
    except Exception as e:
        print(f"读取目录 {base_dir} 时出错: {e}")
        return items
...
def main():
    """主函数 - 增强版本，支持尺寸统一和Windows风格排序"""
    script_dir = get_script_directory()
    print("=== 图片转PDF工具 1.4.2 (Windows风格排序版) ===")
    print(f"工作目录: {script_dir}")
    print("自动扫描并处理当前目录下的zip文件和文件夹")
    print("支持图片尺寸统一功能和Windows风格自然排序\n")
